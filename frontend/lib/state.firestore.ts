...
const getColl = (profileId: string, coll: string) => db.collection('profiles').doc(profileId).collection(coll);
...
export async function createProfile(name: string, type?: string, businessNumber?: string): Promise<Profile> {
  ...
  await db.collection('profiles').doc(id).set(profile);
  return profile;
}
...
export async function getAssets(profileId: string): Promise<Asset[]> {
  const snapshot = await getColl(profileId, 'assets').get();
  return snapshot.docs.map(doc => doc.data() as Asset);
}

export async function saveAssets(profileId: string, assets: Asset[]) {
  const batch = db.batch();
  // Clear existing (optional, but current logic replaces them)
  const existing = await getColl(profileId, 'assets').get();
  existing.docs.forEach(doc => batch.delete(doc.ref));

  assets.forEach(asset => {
    const ref = getColl(profileId, 'assets').doc(asset.id);
    batch.set(ref, { ...asset, profileId });
  });
...
export async function getDebts(profileId: string): Promise<Debt[]> {
  const snapshot = await getColl(profileId, 'debts').get();
  return snapshot.docs.map(doc => doc.data() as Debt);
}

export async function saveDebts(profileId: string, debts: Debt[]) {
  const batch = db.batch();
  const existing = await getColl(profileId, 'debts').get();
  existing.docs.forEach(doc => batch.delete(doc.ref));

  debts.forEach(debt => {
    const ref = getColl(profileId, 'debts').doc(debt.id);
    batch.set(ref, { ...debt, profileId });
  });
...
export async function getExpenses(profileId: string): Promise<RecurringExpense[]> {
  const snapshot = await getColl(profileId, 'expenses').get();
...
export async function saveExpenses(profileId: string, expenses: RecurringExpense[]) {
  const batch = db.batch();
  const existing = await getColl(profileId, 'expenses').get();
  existing.docs.forEach(doc => batch.delete(doc.ref));

  expenses.forEach(exp => {
    const ref = getColl(profileId, 'expenses').doc(exp.id);
    batch.set(ref, { ...exp, profileId });
  });
...
export async function getIncomes(profileId: string): Promise<Income[]> {
  const snapshot = await getColl(profileId, 'incomes').get();
  return snapshot.docs.map(doc => doc.data() as Income);
}

export async function saveIncomes(profileId: string, incomes: Income[]) {
  const batch = db.batch();
  const existing = await getColl(profileId, 'incomes').get();
  existing.docs.forEach(doc => batch.delete(doc.ref));

  incomes.forEach(inc => {
    const ref = getColl(profileId, 'incomes').doc(inc.id);
    batch.set(ref, { ...inc, profileId });
  });
...
export async function saveTransactions(profileId: string, transactions: Transaction[]) {
  const existingSnapshot = await getColl(profileId, 'transactions').get();
  const existingTransactions = existingSnapshot.docs.map(doc => doc.data() as Transaction);

  // ... (keep the logic for duplicate check and cancellation matching, it's independent of storage) ...
  // [Logic starts here]
      ...
      if (newTxn.transactionNumber && existingTxn.transactionNumber) {
        if (newTxn.transactionNumber === existingTxn.transactionNumber) {
          if (newTxn.datetime && existingTxn.datetime) {
            if (newTxn.datetime === existingTxn.datetime) {
              return { isDuplicate: true, confidence: 1.0, reason: "거래번호 + datetime 일치" }
            }
            confidence = 0.7
            reason = "거래번호 일치, 시간 다름"
          } else {
            confidence = 0.8
            reason = "거래번호 일치"
          }
      ...
      if (newTxn.datetime && existingTxn.datetime) {
        ...
        if (timeDiffMinutes <= 5 && newTxn.amount === existingTxn.amount && newTxn.name === existingTxn.name) {
          if (timeDiffMinutes === 0) {
            return { isDuplicate: true, confidence: 1.0, reason: "datetime + 금액 + 항목명 완전 일치" }
          } else {
            confidence = Math.max(confidence, 0.9)
            reason = "시간 5분 이내 + 금액 + 항목명 일치"
          }
        }
      }
        ...
        if (nameSimilarity > 0.9) {
          if (newTxn.datetime && existingTxn.datetime) {
            ...
            if (timeDiff <= 30) {
              ...
              reason = `날짜 + 금액 + 항목명 유사(${Math.round(nameSimilarity * 100)}%) + 시간 30분 이내`
            } else {
              confidence = Math.max(confidence, 0.6)
              reason = `날짜 + 금액 + 항목명 유사(${Math.round(nameSimilarity * 100)}%) + 시간 차이 큼`
            }
          } else {
            confidence = Math.max(confidence, 0.7)
            reason = `날짜 + 금액 + 항목명 유사(${Math.round(nameSimilarity * 100)}%)`
          }
        } else if (nameSimilarity > 0.7) {
          confidence = Math.max(confidence, 0.5)
          reason = `날짜 + 금액 일치, 항목명 유사(${Math.round(nameSimilarity * 100)}%)`
        }
      }
      if (newTxn.date === existingTxn.date && newTxn.amount === existingTxn.amount && newTxn.name !== existingTxn.name) {
        confidence = Math.max(confidence, 0.3)
        reason = "날짜 + 금액 일치, 항목명 다름"
      }
  ...
  function calculateSimilarity(str1: string, str2: string): number {
    ...
    for (const char of shorter) { if (longer.includes(char)) commonChars.add(char) }
    const similarity = commonChars.size / longer.length
    if (longer.includes(shorter) || shorter.includes(longer)) return Math.max(similarity, 0.8)
    return similarity
  }
  ...
  for (const txn of transactions) {
    if (txn.isCancelled || (txn.originalAmount || 0) < 0) {
      const matched = findMatchingTransaction(txn, existingTransactions)
      if (matched) {
        cancelledTransactionIds.add(txn.id)
        matchedOriginalIds.add(matched.id)
      } else {
        const matchedInFile = findMatchingTransaction(txn, transactions)
        if (matchedInFile && matchedInFile.id !== txn.id) {
          cancelledTransactionIds.add(txn.id)
          matchedOriginalIds.add(matchedInFile.id)
        }
      }
    }
  }

  const transactionsToProcess = transactions.filter(txn => !cancelledTransactionIds.has(txn.id))
  // [Logic ends here]

  const batch = db.batch();

  // Remove matched original transactions from Firestore
  matchedOriginalIds.forEach(id => {
    batch.delete(getColl(profileId, 'transactions').doc(id));
  });
  ...
  for (const txn of transactionsToProcess) {
    const checkResult = checkDuplicate(txn, existingTransactions.filter(t => !matchedOriginalIds.has(t.id)))
    if (checkResult.isDuplicate) {
      duplicates.push(txn)
    } else if (checkResult.confidence >= 0.5) {
      ambiguous.push({ ...txn, duplicateCheckConfidence: checkResult.confidence, possibleDuplicate: true, needsReview: true })
    } else {
      processedTransactions.push(txn)
    }
  }

  const toAdd = [...processedTransactions, ...ambiguous];
  toAdd.forEach(txn => {
    const ref = getColl(profileId, 'transactions').doc(txn.id);
    batch.set(ref, { ...txn, profileId });
  });
...
export async function getTransactions(profileId: string, filters?: { startDate?: string; endDate?: string; type?: 'INCOME' | 'EXPENSE' }) {
  let query: admin.firestore.Query = getColl(profileId, 'transactions');
  ...
  return transactions.sort((a: Transaction, b: Transaction) => b.date.localeCompare(a.date))
}

export async function deleteTransaction(profileId: string, transactionId: string): Promise<boolean> {
  await getColl(profileId, 'transactions').doc(transactionId).delete();
  return true
}

export async function deleteAllTransactions(profileId: string) {
  const batch = db.batch();
  const snapshot = await getColl(profileId, 'transactions').get();
...
export async function deleteTransactionNameMappings(profileId: string) {
  await db.collection('settings').doc('transactionNameMappings').delete(); // Global for now
  return true
}
...
export async function getCategories(profileId: string): Promise<Category[]> {
  const snapshot = await getColl(profileId, 'categories').get();
  return snapshot.docs.map(doc => doc.data() as Category);
}

export async function saveCategories(profileId: string, categories: Category[]) {
  const batch = db.batch();
  const existing = await getColl(profileId, 'categories').get();
  existing.docs.forEach(doc => batch.delete(doc.ref));

  categories.forEach(cat => {
    const ref = getColl(profileId, 'categories').doc(cat.id);
    batch.set(ref, { ...cat, profileId });
  });
...
export async function deleteCategory(profileId: string, categoryId: string): Promise<boolean> {
  await getColl(profileId, 'categories').doc(categoryId).delete();
  return true
}

// 전역 가맹점명-카테고리 매핑 조회 (모든 사용자 공유)
// TODO: 추후 다중 사용자 지원 시 사용자별 매핑도 확인하도록 개선 필요
// 우선순위: 사용자별 매핑 > 전역 매핑
export async function getMerchantCategoryMapping(merchantName: string, profileId?: string): Promise<string | null> {
  ...
  return data?.[normalizedName] || null;
}

// 전역 가맹점명-카테고리 매핑 저장 (AI API 결과 저장, 모든 사용자 공유)
// TODO: 추후 다중 사용자 지원 시 사용자별 매핑 저장 옵션 추가 필요
...
export async function getKeywordCategoryMapping(keyword: string, profileId?: string): Promise<string | null> {
  ...
  const data = doc.data() as Record<string, string>;
  return data?.[normalizedKeyword] || null;
}
...
// 모든 키워드-카테고리 매핑 조회
// TODO: 추후 다중 사용자 지원 시 사용자별 매핑도 포함하도록 개선 필요
...
// 키워드-카테고리 매핑 삭제
// TODO: 추후 다중 사용자 지원 시 사용자별 매핑 삭제 옵션 추가 필요
export async function deleteKeywordCategoryMapping(keyword: string, profileId?: string): Promise<void> {
  const normalizedKeyword = keyword.trim().toLowerCase();
  const ref = db.collection('settings').doc('keywordCategoryMappings');
  await ref.update({
    [normalizedKeyword]: admin.firestore.FieldValue.delete()
  });
}
...
export async function replaceTransactions(profileId: string, transactions: Transaction[]) {
  const batch = db.batch();
  const existing = await getColl(profileId, 'transactions').get();
  existing.docs.forEach(doc => batch.delete(doc.ref));
